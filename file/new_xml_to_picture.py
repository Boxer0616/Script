...
parser.add_argument('-i', '--input_dir',  default = '/home/hiroshi/M2Det_H/dataset/val')
parser.add_argument('-ii', '--input_dir2',  default = '/var/samba/追加実験/選定/Result/Mask/val')
parser.add_argument('-o', '--out_dir',  default = '/var/samba/追加実験/選定/Result/Mask/draw')
    ...
    for img_path in tqdm(img_path_list):
        ...
        for xml_path in xml_path_list:
            for label in get_label(xml_path, CLASSES):
                labe = label.split(' ')
                framenamex = xml_path.rsplit('/', 1)[1].rsplit('.', 1)[0]
                l = framenamex
                xmin = int(labe[0])
                ymin = int(labe[1])
                xmax = int(labe[2])
                ...
                if l==framename:
                    ...
                    if labe[4] == 'green_field':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='lawngreen', width=2)
                        d.text((xmin, ymin),'green_field',fill='lawngreen')
                    if labe[4] == 'field':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='red', width=2)
                        d.text((xmin, ymin),'field',fill='red')
                    if labe[4] == 'greenhouse':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='blue', width=2)
