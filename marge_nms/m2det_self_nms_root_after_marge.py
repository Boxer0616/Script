...
parser.add_argument('-p', '--pre_xml',  default = './output-False-nms/marge512-320-800-0.6-0.6-0.6')
parser.add_argument('-d', '--dir_name', default = 'supervised')
parser.add_argument('-s', '--save_dir', default = 'marge512-320-800-0.6-0.6-0.6_0.9')
parser.add_argument('-i', '--draw_img', default = '/home/hiroshi/Dataset/folda-change/20191011矩形最終/val')
parser.add_argument('-o', '--out_dir',  default = './output-nms/marge512-320-800-0.6-0.6-0.6_0.9')
...
--------------------------------------------------------------------------------------------
"""
def get_xmlbox(xml_path, category):
    box_list = []
...
if __name__ == '__main__':
    ...
    for img_path in img_list:
        ...
        xml_list = glob.glob(os.path.join(args.pre_xml, directory, 'supervised' ,framename + '*.xml'))

        all_box_list = []

        for xml in xml_list:
            # xmlboxを取得
            box_list = get_xmlbox(xml, category)
            all_box_list.extend(box_list)

        #selected_box_list = non_max_suppression(all_box_list, overlap_thresh=0.9)
        selected_box_list = non_max_suppression_z(all_box_list, overlap_thresh=0.9)
