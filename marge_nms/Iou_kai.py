...
parser.add_argument('-ii', '--input_dir2',  default = '/home/hiroshi/Test/test/PR_Curve/output-nms/sfam320-512-800/draw-nms-0.6')
...
def create_xml(obj,out_dir):
    if len(obj)!=0:
        ...
        #改行
        indent(root)
        create_dir(out_dir,directory)
        #xml保存
        tree.write(os.path.join(out_dir, directory) + '/' + 'supervised' +'/'+ obj[0]['filename']  + '.xml', encoding="utf-8", xml_declaration=True)

#detection_ver_xmlファイル作成
def create_xml_detection(obj,out_dir):
    if len(obj)!=0:
        #nn = obj[0]['filename'].rsplit("_")
        #nn = obj[0]['boxA']['filename'].rsplit("_")
        #file = nn[0] + '_' + nn[1] + '_' + nn[2] + '_' + nn[3]	
        ...
        create_dir(out_dir,directory)
        #xml保存
        tree.write(os.path.join(out_dir, directory) + '/'+ 'supervised' +'/'+ obj[0][0]['boxA']['filename']  + '.xml', encoding="utf-8", xml_declaration=True)

#ディレクトリ作成
def create_dir(out_dir,directory):
    # ディレクトリがなければ作成
    if not (os.path.isdir(os.path.join(out_dir, directory,'supervised'))):
        os.makedirs(os.path.join(out_dir, directory,'supervised'))
...
if __name__ == '__main__':
    ...
    for pp,obb in enumerate(notdetection_box_list2):
        #xml作成
    	create_xml(obb,args.out_dir+'/mike')
    for pp,obb in enumerate(detection_box_list):
        #xml作成
    	create_xml_detection(obb,args.out_dir+'/ken')
