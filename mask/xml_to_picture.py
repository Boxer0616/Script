...
parser.add_argument('-i', '--input_dir',  default = './dataset/miken')
#parser.add_argument('-i', '--input_dir',  default = '../dataset/val')
parser.add_argument('-o', '--out_dir',  default = './draw')
...
if __name__ == '__main__':
    ...
    xml_path_list = glob.glob(os.path.join(args.input_dir,'xml', '**' ,'*.xml'))
        ...
        for xml_path in xml_path_list:
            for label in get_label(xml_path, CLASSES):
                ...
                if l==framename:
                    ...
                    #d.text((xmin, ymin),'green_field',fill='red')
                    if labe[4] == 'green_field':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='red', width=5)
                        d.text((xmin, ymin),'green_field',fill='red')
                        #cv2.putText(d,'green_field', (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), thickness=2)
                    if labe[4] == 'field':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='lawngreen', width=5)
                        d.text((xmin, ymin),'field',fill='lawngreen')
                    if labe[4] == 'greenhouse':
                        d.rectangle([(xmin, ymin), (xmax, ymax)], outline='blue', width=5)
